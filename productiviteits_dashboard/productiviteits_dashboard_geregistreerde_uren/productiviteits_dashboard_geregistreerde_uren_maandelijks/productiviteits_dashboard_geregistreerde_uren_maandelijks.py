...
from productiviteits_dashboard_geregistreerde_uren_maandelijks_modules.database import connect_to_database, write_to_database, clear_table
from productiviteits_dashboard_geregistreerde_uren_maandelijks_modules.config import fetch_configurations, fetch_script_id, fetch_all_connection_strings
from productiviteits_dashboard_geregistreerde_uren_maandelijks_modules.table_mapping import transform_columns, geregistreerde_uren
from productiviteits_dashboard_geregistreerde_uren_maandelijks_modules.type_mapping import convert_column_types, geregistreerde_uren_typing
from productiviteits_dashboard_geregistreerde_uren_maandelijks_modules.column_selection import select_columns, geregistreerde_uren_columns
from productiviteits_dashboard_geregistreerde_uren_maandelijks_modules.get_request import get_request
import pandas as pd
import logging
...
def main():
    ...
    script = "Geregistreerde Uren | Maandelijks"
        ...
        if klantnaam == "Borst Bloembollen":
            ...
            for bron, dict in configuratie_dict.items():
                if bron == 'Dyflexis | Azure SQL Database':
                    ...
                    twee_jaar_terug = huidig_jaar - 2
                    begin_jaar = 2019

                    # Datum range maken voor alle jaren van begin_jaar tot en met twee_jaar_terug
                    datum_range = {}
                    for jaar in range(begin_jaar, twee_jaar_terug + 1):
                        datum_range[jaar] = [f"{jaar}-01-01", f"{jaar}-12-31"]
